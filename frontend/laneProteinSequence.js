...
function ProteinSequenceLane(graphLayout,
  ...
  this.exonElements = this.exons.selectAll("rect").data(exons)
    ...
    .attr("style", function (d) {
      return d.noData ? "fill:rgba(255,255,255,1)" : d.odd ? "fill:rgba(117,  25, 117, 0.3)" : "fill:rgba(224, 163, 102, 0.3)"
    });
  ...
  this.letter.numberBox = 
      this.letter.group.append("rect")
          .attr("id", "aa-number-rect")
          .attr("y", laneLayout.y - 1.25 * laneLayout.height)
          .attr("height", laneLayout.height)
          .attr("width", laneLayout.height)

  this.letter.number = 
      this.letter.group.append("text")
          .attr("id", "aa-number-text")
          .attr("y", (laneLayout.y - 1.25 * laneLayout.height/2))
}

ProteinSequenceLane.prototype = {
  appendDetails: function (element, aminoAcid) {
    element.append("<h4>AA #" + aminoAcid + " - <span class='aa-letter'>" + this.seq[aminoAcid - 1] + "</span></h4>")
  },
  ...
  update: function () {
    ...
    for (var i = Math.ceil(xScale.domain()[0]) - 1; i < xScale.domain()[1]; i++) {
      ...
      if (xPos - prevDisplayedX > 10 && this.seq) {
        var text = this.seq[i];
        var ellipsis = false;
